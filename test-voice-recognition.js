/**
 * Test Runner for Voice Recognition
 * 
...
async function runTests() {
    ...
    sessionManager.logger = new SimpleLogger('SessionManager');
    ...
    try {
        ...
        console.log(`✅ Loaded ${sessionManager.setCards.get(testSet.set_name)?.length || 0} cards from set`);
        ...
        for (const testCase of TEST_CASES) {
            ...
            try {
                console.log(`\n🔍 [${new Date().toISOString()}] Testing: "${input}"`);
                ...
                const matchedName = bestMatch.card.name;
                const score = bestMatch.score.toFixed(2);
