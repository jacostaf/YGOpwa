...
export class SessionManager {
    ...
    createEmptySession(setName = 'Unknown Set') {
        return {
     ...
     */
    getApiUrl() {
        // Default to the Phase 3 aggregation layer
        return config.API_URL || 'http://127.0.0.1:8081/api/v1';

    }

    /**
     * Perform a lightweight health check so we fail fast when the backend is offline
     */
    async verifyBackendHealth() {
        if (this.healthCheckVerified || !this.config.enableHealthCheck || isTestEnvironment) {
            return true;
        }

        const trimmedBase = (this.apiUrl || '').replace(/\/$/, '');
        const healthUrl = `${trimmedBase || 'http://127.0.0.1:8081/api/v1'}/health`;
        const timeout = Math.min(this.config.healthCheckTimeout || 5000, this.config.apiTimeout || 120000);
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), timeout);

        this.logger.info(`[API DEBUG] Checking backend health at ${healthUrl}`);
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`HTTP ${response.status}: ${response.statusText} while calling ${healthUrl}`);
            }

            this.healthCheckVerified = true;
            this.logger.info(`[API DEBUG] Backend health check passed for ${healthUrl}`);
            return true;
        } catch (error) {
            clearTimeout(timeoutId);
            const errorMessage = `Backend API health check failed at ${healthUrl}. ` +
                'Start tcgcsv_server.py and confirm runtime-env.js VITE_CARD_API_BASE_URL matches the backend port.';
            this.logger.error('[API DEBUG] Backend health check failed:', error);
    ...
    async initialize(storage) {
        ...
        try {
            if (this.config.enableHealthCheck && !isTestEnvironment) {
    ...
    async loadCardSets(searchTerm = '') {
        try {
            ...
            const normalizedBase = (this.apiUrl || 'http://127.0.0.1:8081/api/v1').replace(/\/$/, '');
            const targetEndpoint = this.searchTerm
                ? `${normalizedBase}/card-sets/search/${encodeURIComponent(this.searchTerm)}`
                : `${normalizedBase}/card-sets/from-cache`;
            const envHint = 'Check runtime-env.js → VITE_CARD_API_BASE_URL and ensure the backend is running before launching the PWA.';

            if (error.message.includes('backend') || error.message.includes('connect')) {
                errorMessage = `Cannot connect to backend API at ${targetEndpoint}. ${envHint}`;
            } else if (error.message.includes('timeout')) {
                errorMessage = `Request to ${targetEndpoint} timed out. ${envHint}`;
            } else {
                errorMessage = `Failed to load card sets from ${targetEndpoint}: ${error.message}`;
            }
    ...
    async fetchCardSets(searchTerm = '') {
        try {
            ...
            if (!response.ok) {
                throw new Error(`HTTP ${response.status}: ${response.statusText} while calling ${url}`);
            }
            ...
            const envHint = 'Check runtime-env.js → VITE_CARD_API_BASE_URL and ensure the backend is running.';
            ...
            if (error.message.includes('fetch') || error.message.includes('NetworkError')) {
                ...
                throw new Error(`Cannot connect to backend API at ${url}. ${envHint}`);
            }

            if (error.message.includes('ECONNREFUSED')) {
                this.logger.error('[API DEBUG] Connection refused - backend server is not listening on port 8081');
                throw new Error(`Connection refused at ${url}. ${envHint}`);
            }

            throw new Error(`Failed to fetch card sets from ${url}: ${error.message}`);
        }
    }
    ...
    async fetchEnhancedCardInfo(cardData) {
        try {
            ...
            // TCGcsv backend returns price data wrapped in data property
            return data.data;
    ...
    async addCard(cardData, forceRefreshPricing = false) {
        const hasValidQuantity = cardData?.quantity === undefined || Number(cardData.quantity) > 0;
        const resolvedSetCode = cardData?.setCode
            || cardData?.set_code
            || cardData?.code
            || cardData?.setInfo?.setCode
            || cardData?.packCode;
        const resolvedSetName = cardData?.set
            || cardData?.set_name
            || cardData?.setName
            || cardData?.setInfo?.setName
            || cardData?.packName
            || this.currentSession?.setName
            || this.currentSet?.name;
        const isValidCard = cardData
            && cardData.name
            && hasValidQuantity
            && (resolvedSetCode || resolvedSetName || cardData.packName);
        if (!isValidCard) {
            this.logger.warn('Invalid card data (missing set or code):', {
                name: cardData?.name,
                set: cardData?.set,
                set_name: cardData?.set_name,
                setCode: cardData?.setCode,
                set_code: cardData?.set_code,
                packName: cardData?.packName
            });
            return false;
        }

        // Normalize set metadata so downstream UI always has consistent fields
        if (resolvedSetCode) {
            cardData.setCode = cardData.setCode || resolvedSetCode;
            cardData.set_code = cardData.set_code || resolvedSetCode;
            cardData.code = cardData.code || resolvedSetCode;
        }
        if (resolvedSetName) {
            cardData.set = cardData.set || resolvedSetName;
            cardData.set_name = cardData.set_name || resolvedSetName;
            cardData.setName = cardData.setName || resolvedSetName;
            cardData.packName = cardData.packName || resolvedSetName;
        }
        ...
        try {
            ...
            // Normalize image fields from tcgcsv/tcgplayer style responses
            if (!cardData.image_url && cardData.imageUrl) {
                cardData.image_url = cardData.imageUrl;
            }
            if (!cardData.image_url_small && cardData.imageUrlSmall) {
                cardData.image_url_small = cardData.imageUrlSmall;
            }
            ...
            const enhancedCard = {
                ...
                ...cardData,
                imageUrl: cardData.imageUrl || cardData.image_url || cardData.image_url_small || null,
                image_url: cardData.image_url || cardData.imageUrl || null,
                image_url_small: cardData.image_url_small || cardData.imageUrlSmall || null
            };
    ...
    async _updateCardPricing(card, cardData) {
        ...
        try {
            ...
            this.logger.info('Enhanced info received:', enhancedInfo);

            if (enhancedInfo) {
                this._applyPricingData(card, {
                    tcg_price: enhancedInfo.low_price || enhancedInfo.tcg_price,
                    tcg_market_price: enhancedInfo.tcg_market_price,
                    price: parseFloat(enhancedInfo.tcg_market_price || enhancedInfo.low_price || enhancedInfo.tcg_price || '0'),
                    price_status: 'loaded',
                    ...
                    image_url_small: enhancedInfo.image_url || card.image_url_small,
    ...
    _applyPricingData(card, pricingData) {
        ...
        const imageUrl = pricingData.imageUrl || pricingData.image_url || card.imageUrl || card.image_url || null;
        const imageUrlSmall = pricingData.image_url_small || pricingData.imageUrlSmall || card.image_url_small || card.imageUrlSmall || null;
    ...
    removeCard(cardIdOrIndex) {
        ...
        if (typeof cardIdOrIndex === 'number') {
            ...
            cardIndex = this.currentSession.cards.findIndex(card => card.id === cardIdOrIndex);
        }
        if (cardIndex === -1) {
            throw new Error('Card not found in session');
        }
        const cardId = this.currentSession.cards[cardIndex]?.id;
    ...
    async refreshCardPricing(cardId) {
        ...
        try {
            ...
            try {
                const enhancedInfo = await this.fetchEnhancedCardInfo(card);
                if (enhancedInfo) {
                    const pricingData = enhancedInfo.data || enhancedInfo;
                    const lowPrice = pricingData.tcg_price || pricingData.tcgPrice || pricingData.lowPrice;
                    const marketPrice = pricingData.tcg_market_price || pricingData.marketPrice || pricingData.highPrice;
                    const scrapeSuccess = pricingData.scrape_success ?? pricingData.scrapeSuccess ?? true;
                    const lastUpdate = pricingData.last_price_updt || pricingData.lastPriceUpdate || enhancedInfo.last_price_updt;

                    // Update the existing card with fresh pricing data
                    Object.assign(card, {
                        // Price information
                        tcg_price: lowPrice,
                        tcg_market_price: marketPrice,
                        price: parseFloat(marketPrice || lowPrice || '0'),

                        // Update pricing status
                        price_status: 'refreshed',
                        last_price_updt: lastUpdate || new Date().toISOString(),
                        importedPricing: false, // No longer using imported pricing

                        // Update other enhanced info if available
                        source_url: pricingData.source_url || card.source_url,
                        scrape_success: scrapeSuccess
                    });

                    this.logger.info(`Pricing refreshed for: ${card.name} - New TCG Low: $${lowPrice || 'N/A'}, New TCG Market: $${marketPrice || 'N/A'}`);
                } else {
    ...
    calculateRarityScore(inputRarity, cardSetRarity) {
        ...
        if (input.includes(cardRarity) || cardRarity.includes(input)) {
            ...
            if (inputWords.every(word => cardWords.includes(word)) && inputWords.length < cardWords.length) {
                const missingWords = cardWords.filter(word => !inputWords.includes(word));
                const genericWords = ['rare', 'holo', 'foil', 'quart', 'quarter', 'century'];
                const onlyGenericMissing = missingWords.every(word => genericWords.includes(word));
                if (!onlyGenericMissing) {
                    return 40; // Lower score for incomplete rarity specification with missing key descriptors
                }
            }
    ...
    buildLegacySessionSnapshot() {
        if (!Array.isArray(this.cards) || this.cards.length === 0) {
            return null;
        }

        const statistics = typeof this.calculateSessionStatistics === 'function'
            ? this.calculateSessionStatistics(this.cards)
            : { totalCards: this.cards.length };

        return {
            id: this.sessionId || `legacy-session-${Date.now()}`,
            setName: this.packName || 'Unknown Set',
            startTime: new Date().toISOString(),
            endTime: null,
            statistics,
     ...
     */
    exportSession(format = 'json', selectedFields = null, waitForPricing = true) {
        let session = this.currentSession;

        if (!session) {
            session = this.buildLegacySessionSnapshot();
            if (!session) {
                const error = new Error('No active session to export');
                if (format === 'json' || format === 'csv' || format === 'excel') {
                    return Promise.reject(error);
                }
                throw error;
            }
            this.logger?.warn?.('Exporting session without active currentSession; using legacy snapshot data');
        }
        ...
        const buildExport = () => {
            ...
            if (format === 'csv') {
                return this.exportSessionToCSV(selectedFields, session);
            } else if (format === 'excel') {
                return this.exportSessionToExcel(selectedFields, session);
            }
            ...
            if (format === 'json') {
                ...
                const enriched = new String(jsonString);
                Object.assign(enriched, payload);
                enriched.toJSON = () => payload;
                enriched.raw = jsonString;
                return enriched;
            }
        ...
        if (session === this.currentSession && waitForPricing && this.loadingPriceData.size > 0) {
            this.logger.info('Export waiting for pricing data to finish loading...');
            return this.waitForPricingDataToLoad().then((pricingLoaded) => {
                if (!pricingLoaded) {
     ...
     */
    exportSessionToCSV(selectedFields = null, session = this.currentSession) {
            ...
            return fields.map(field => {
                ...
                switch (field) {
                    ...
                    case 'rarity':
                        ...
                        break;
                    case 'setCode':
                        value = card.set_code || card.setCode || session.setId || '';
                        break;
                    case 'cardNumber':
        ...
        return {
            content: csvContent,
            filename: `YGO_Session_${(session.setName || 'session').toString().replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`,
            mimeType: 'text/csv'
        };
    }
     ...
     */
    exportSessionToExcel(selectedFields = null, session = this.currentSession) {
        ...
        const csvExport = this.exportSessionToCSV(selectedFields, session);
    ...
    async generateExportFile(format = 'json', selectedFields = null, waitForPricing = true) {
        ...
        if (format === 'csv') {
            ...
            // JSON format
            const jsonString = typeof exportData?.raw === 'string'
                ? exportData.raw
                : JSON.stringify(exportData, null, 2);
            content = jsonString;
            const baseName = this.currentSession?.setName || exportData?.setName || 'session';
            filename = `YGO_Session_${baseName}_${new Date().toISOString().split('T')[0]}.json`;
    ...
    calculateSessionStatistics(cards) {
        ...
        cards.forEach(card => {
            ...
            this.logger.debug(`Calculating stats for card: ${card.name}`, {
                tcg_price: card.tcg_price,
                tcg_market_price: card.tcg_market_price,
                price: card.price,
                type_tcg: typeof card.tcg_price,
                type_market: typeof card.tcg_market_price
            });
            ...
            if (!isNaN(basePrice)) {
                stats.totalValue += basePrice * quantity;
            }
        ...
        if (stats.totalCards > 0) {
            const valueBasis = stats.tcgMarketTotal || stats.tcgLowTotal || stats.totalValue;
            stats.averageValue = valueBasis / stats.totalCards;
        }

        this.logger.info(`Session statistics calculated: ${stats.totalCards} cards, Total Value: $${stats.totalValue.toFixed(2)}, TCG Low Total: $${stats.tcgLowTotal.toFixed(2)}, TCG Market Total: $${stats.tcgMarketTotal.toFixed(2)}`);
        return stats;
    }

    getSessionStats() {
        const sourceCards = Array.isArray(this.currentSession?.cards) && this.currentSession.cards.length
            ? this.currentSession.cards
            : Array.isArray(this.cards)
                ? this.cards
                : [];

        const stats = this.calculateSessionStatistics(sourceCards);
        const valueBasis = stats.tcgMarketTotal || stats.tcgLowTotal || stats.totalValue;

        return {
            totalCards: stats.totalCards,
            totalValue: valueBasis,
            averageValue: stats.totalCards > 0 ? Number(valueBasis / stats.totalCards) : 0,
            rarityCounts: stats.rarityBreakdown
        };
    }
