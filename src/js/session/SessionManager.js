...
export class SessionManager {
    ...
    async initialize(storage) {
        ...
        const logStep = (step, message) => {
            ...
            this.logger.debug(`[SESSION_INIT] [${elapsed}s] ${step}: ${message}`);
            console.log(`[SESSION_INIT] [${elapsed}s] ${step}: ${message}`);
        };

        try {
            logStep('START', 'Initializing session manager...');
            
            if (storage) {
                logStep('STORAGE', 'Setting storage instance');
                this.storage = storage;
            } else {
                logStep('WARN', 'No storage instance provided');
            }
            
            try {
                // Load card sets
                logStep('CARD_SETS', 'Loading card sets...');
                await this.loadCardSets().catch(err => {
                    logStep('ERROR', `Failed to load card sets: ${err.message}`);
                    throw new Error(`Failed to load card sets: ${err.message}`);
                });
                logStep('CARD_SETS', 'Card sets loaded successfully');
                
                // Load last session if available
                logStep('SESSION_LOAD', 'Loading last session...');
                await this.loadLastSession().catch(err => {
                    logStep('WARN', `Failed to load last session: ${err.message}`);
                    // Continue even if loading last session fails
                });
                
                const totalTime = ((performance.now() - startTime) / 1000).toFixed(2);
                logStep('COMPLETE', `Session manager initialized successfully in ${totalTime}s`);
                return true;
                
            } catch (error) {
                const errorTime = ((performance.now() - startTime) / 1000).toFixed(2);
                logStep('ERROR', `Initialization failed after ${errorTime}s: ${error.message}`);
                throw error;
            }
            
        } catch (error) {
            const errorTime = ((performance.now() - startTime) / 1000).toFixed(2);
            this.logger.error(`Failed to initialize session manager after ${errorTime}s:`, error);
            console.error(`[SESSION_INIT] Failed after ${errorTime}s:`, error);
            throw error;
        }
    }
    ...
    async loadCardSets(searchTerm = '') {
        ...
        const logStep = (step, message) => {
            const elapsed = ((performance.now() - startTime) / 1000).toFixed(2);
            this.logger.debug(`[LOAD_SETS] [${elapsed}s] ${step}: ${message}`);
            console.log(`[LOAD_SETS] [${elapsed}s] ${step}: ${message}`);
        };

        try {
            this.searchTerm = searchTerm.trim();
            logStep('START', `Loading card sets${this.searchTerm ? ` with search: "${this.searchTerm}"` : ' (all sets)'}`);
            ...
            if (this.searchTerm) {
                logStep('SEARCH', 'Search term provided, fetching from API');
                // Always fetch search results from API
                sets = await this.fetchCardSets(this.searchTerm);
            } else {
                logStep('CACHE', 'No search term, checking cache first');
                // Try to load full set list from storage first (for performance)
                const cacheStart = performance.now();
                const [cachedSets, cacheTimestamp] = await Promise.all([
                    this.storage?.get('cardSets'),
                    this.storage?.get('cardSetsTimestamp')
                ]);
                
                const cacheTime = performance.now() - cacheStart;
                logStep('CACHE', `Cache check completed in ${cacheTime.toFixed(2)}ms`);
                
                const now = Date.now();
                const cacheMaxAge = 24 * 60 * 60 * 1000; // 24 hours
                
                // Check cache validity
                const isCacheValid = cachedSets && 
                    Array.isArray(cachedSets) && 
                    cachedSets.length > 100 &&
                    cacheTimestamp && 
    ...
    /**
     * Find cards in the current set that match the given card name
     * @param {string} cardName - The card name to search for
     * @param {string} [rarity] - Optional rarity filter
     * @returns {Promise} Promise that resolves to an array of matching cards with confidence scores
     */
    /**
     * Find cards in the current set that match the given card name using fuzzy matching
     * @param {string} cardName - The card name to search for
     * @param {string} [rarity] - Optional rarity filter
     * @returns {Promise} Promise that resolves to an array of matching cards with confidence scores
     */
    findCardsInCurrentSet(cardName, rarity) {
        rarity = rarity || null;
        
        this.logger.debug('[FIND_CARDS] Searching for card in current set: "' + cardName + '"' + (rarity ? ' with rarity: ' + rarity : ''));
        
        if (!this.currentSet) {
            this.logger.warn('No current set selected');
            return Promise.resolve([]);
        }
        
        // Get cards from the current set
        const setCards = this.setCards.get(this.currentSet.id) || [];
        this.logger.debug(`[FIND_CARDS] Searching in set: ${this.currentSet.name} (${setCards.length} cards)`);
        
        if (setCards.length === 0) {
            this.logger.warn('No cards loaded for current set');
            return Promise.resolve([]);
        }
        
        try {
            // Generate variants for the search term
            const searchVariants = this.generateCardNameVariants(cardName);
            this.logger.debug(`[FIND_CARDS] Generated ${searchVariants.length} search variants`);
            
            if (searchVariants.length === 0) {
                this.logger.warn('No search variants generated');
                return Promise.resolve([]);
            }
            ...
            for (const card of setCards) {
                // Skip if rarity filter is provided and doesn't match
                if (rarity && card.rarity && !card.rarity.toLowerCase().includes(rarity.toLowerCase())) {
                    continue;
                }
                
                // Generate variants for the card name
                ...
                const lengthDifference = Math.abs(cardName.length - card.name.length);
            ...
            } // End of for (const card of setCards) loop
            ...
            this.logger.debug('[FIND_CARDS] Found ' + matches.length + ' matches');
            return Promise.resolve(matches);
            
        } catch (error) {
            this.logger.error('Error in findCardsInCurrentSet:', error);
            return Promise.resolve([]);
        }
    }
    ...
    async processVoiceInput(transcript) {
        ...
        try {
            ...
            if (recognizedCards.length > 0) {
                ...
                this.logger.warn(`No cards found for transcript: "${cleanTranscript}"`);
            }
    ...
    /**
     * Generate card name variants for better matching
     * Enhanced to handle complex card names and common mispronunciations
     */
    generateCardNameVariants(name) {
        ...
        // Common Yu-Gi-Oh! specific substitutions and phonetic variants
        const substitutions = {
            // General card terms
            'yu': ['you', 'u', 'yoo'],
            'gi': ['gee', 'ji', 'ghee'],
            'oh': ['o', 'oh', 'ou'],
            'elemental': ['elemental', 'element', 'elemnt'],
            'hero': ['hiro', 'heero', 'hero', 'herro'],
            'evil': ['evil', 'evel', 'evl'],
            'dark': ['dark', 'darc', 'dak'],
            'gaia': ['gaia', 'gaya', 'guy', 'gya', 'guy-ah'],
            'cyber': ['siber', 'cyber', 'syber'],
            'dragon': ['dragoon', 'dragun', 'dragen', 'draggin', 'dragon'],
            'magician': ['magic', 'majishan', 'majician', 'majishun'],
            'warrior': ['war', 'worrier', 'warrior'],
            'machine': ['machin', 'mashine', 'machien'],
            'beast': ['beast', 'beest', 'best'],
            'fiend': ['fend', 'feend', 'fiend'],
            'spellcaster': ['spell', 'caster', 'spell caster'],
            'aqua': ['agua', 'aqua', 'akwa'],
            'winged': ['wing', 'wingd', 'winged'],
            'thunder': ['thunda', 'thuner', 'thunder'],
            'zombie': ['zomb', 'zombee', 'zomby'],
            'plant': ['plan', 'plnt', 'plant'],
            'insect': ['insec', 'insect', 'insek'],
            'rock': ['rok', 'rokk', 'rock'],
            'pyro': ['pyro', 'piro', 'piroh'],
            'sea': ['see', 'sea', 'cee'],
            'divine': ['divin', 'divyne', 'divine'],
            'metal': ['metal', 'mettal', 'metl'],
            'flame': ['flame', 'flaim', 'flaym'],
            'neos': ['neos', 'neeos', 'neus', 'nee-ohs'],
            
            // Specific card name parts
            'dead': ['ded', 'dead', 'dedd'],
            'end': ['end', 'n', 'nd'],
            'prison': ['prisn', 'prison', 'prizon'],
            'tenyi': ['teny', 'ten-yi', 'tenyee', 'teny-eye'],
            'infinity': ['infinity', 'infiniti', 'infinite', 'inf'],
            'futsu': ['futsu', 'footsoo', 'futsoo', 'futsuu'],
            'no': ['no', 'know', 'now'],
            'mitama': ['mitama', 'meetama', 'meetahma', 'mitamah'],
            'mitsurugi': ['mitsurugi', 'mitsurugui', 'mitsurugy', 'mitsuruguee'],
            'supreme': ['supreme', 'soopreem', 'supreem'],
            'darkness': ['darkness', 'darknes', 'darknesu']
        };
        
        // Generate variants with substitutions - optimized to prevent stack overflow
        const generateWithSubs = (current, index = 0, depth = 0, maxDepth = 20) => {
            // Safety check to prevent infinite recursion
            if (depth > maxDepth || index >= current.length) {
                const variant = current.join(' ').replace(/\s+/g, ' ').trim();
            ...
            if (!word) {
                generateWithSubs(current, index + 1, depth + 1, maxDepth);
                return;
            }
            
            let foundSub = false;
            
            // Check if this word or part of it has substitutions
            for (const [original, alts] of Object.entries(substitutions)) {
                if (word.includes(original)) {
                    foundSub = true;
                    for (const alt of alts) {
                        const newWords = [...current];
                        newWords[index] = word.replace(original, alt);
                        // Only proceed if we haven't seen this exact word sequence before
                        const key = newWords.join(' ');
                        if (!variants.has(key)) {
                            generateWithSubs(newWords, index, depth + 1, maxDepth);
                        }
                    }
                }
            }
            
            // Only try removing very short words to prevent explosion of variants
            if (word.length <= 3) {
                const withoutWord = [...current];
                withoutWord.splice(index, 1);
                const key = withoutWord.join(' ');
                if (!variants.has(key)) {
                    generateWithSubs(withoutWord, index, depth + 1, maxDepth);
                }
            }
            
            // Move to next word if we didn't find any substitutions
            if (!foundSub) {
                generateWithSubs(current, index + 1, depth + 1, maxDepth);
            }
        };
        ...
        generateWithSubs(words);
        
        // Add common mispronunciations for the full name
        const commonMispronunciations = {
            'evil hero dead end prison': ['evil hero dead end', 'evil hero deadend prison', 'evil hero deadend'],
            'tenyinfinity': ['tenyi infinity', 'teny infinity', 'teny-infinity', 'teny infinity'],
            'futsu no mitama no mitsurugi': ['futsu no mitama mitsurugi', 'futsu mitama mitsurugi', 'futsunomitamanomitsurugi']
        };
        
        if (commonMispronunciations[lowerName]) {
            commonMispronunciations[lowerName].forEach(variant => variants.add(variant));
        }
        
        // Add compound word variants (with and without spaces/hyphens)
        const baseName = lowerName.replace(/[\s-]+/g, '');
        variants.add(baseName);
        
        // Add versions with common separators
        if (baseName.length > 5) {
            // Add version with spaces between words (if not already added)
            const spaced = baseName.replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase();
            variants.add(spaced);
        ...
        // Add common abbreviations (e.g., 'BEWD' for 'Blue-Eyes White Dragon')
        if (lowerName === 'blue-eyes white dragon') {
            variants.add('bewd');
            variants.add('blue eyes');
        } else if (lowerName === 'dark magician') {
            variants.add('dm');
        } else if (lowerName === 'red-eyes black dragon') {
            variants.add('rebd');
            variants.add('red eyes');
        }
     ...
     * Handles common variations in Yu-Gi-Oh card naming with improved handling of special cases
     */
    normalizeCardName(name) {
        ...
        // Handle common Yu-Gi-Oh! specific patterns
        const patterns = [
            // Handle special characters and common mispronunciations
            [/[\u2018\u2019\u201A\u201B\u2032\u2035\u02BC\u02BB']/g, ''], // Various apostrophe-like characters
            [/[\u201C\u201D\u201E\u201F\u2033\u2036"]/g, ''], // Various quote characters
            [/[\u2013\u2014\u2015\u2212\u2500\u2E3A\u2E3B\uFE58\uFE63\uFF0D\uFF70\uFFBE-\uFFDF]/g, '-'], // Various dash/hyphen characters
            [/[^a-z0-9\s-]/g, ' '], // Replace other special characters with spaces
            
            [/\b(?:evil\s*hero|evilhero)\b/gi, 'evilhero'],
            [/\b(?:elemental\s*hero|elementalhero|e\s*hero|ehero)\b/gi, 'elementalhero'],
            [/\b(?:destiny\s*hero|destinyhero|d\s*hero|dhero)\b/gi, 'destinyhero'],
            [/\b(?:vision\s*hero|visionhero|v\s*hero|vhero)\b/gi, 'visionhero'],
            
            // Handle specific card names that are often mispronounced
            [/\bpot of greed\b/gi, 'potofgreed'],
            [/\bblue\s*eyes\s*white\s*dragon\b/gi, 'blueeyeswhitedragon'],
            [/\bred\s*eyes\s*black\s*dragon\b/gi, 'redeyesblackdragon'],
            [/\bdark\s*magician\s*girl\b/gi, 'darkmagiciangirl'],
            
            // Handle Japanese/English name variations
            [/\b(?:harpie|harper|harper's|harpy)\b/gi, 'harpie'],
            [/\b(?:fissure|fission|fisher|fissure)\b/gi, 'fissure'],
            [/\b(?:mystical|mystic|mistical)\s*(?:space|space typhoon|typhoon)\b/gi, 'mysticalspacetyphoon'],
            
            // Handle specific problematic card names
            [/\b(?:tenyi|tenyinfinity|teny infinity|ten yin|ten yin infinity|tenyin infinity)\b/gi, 'tenyi'],
            [/\b(?:futsu no mitama no mitsurugi|futsu mitama mitsurugi|futsumitamamitsurugi|futsu no mitama mitsurugi|futsu mitama no mitsurugi)\b/gi, 'futsunomitamanomitsurugi'],
            [/\b(?:evil hero dead end prison|evilhero deadendprison|evil hero deadend prison|evilhero dead end prison)\b/gi, 'evilherodeadendprison']
        ];
        ...
        this.logger.debug(`[NORMALIZE] "${name}" -> "${normalized}"`);
        return normalized;
    }
